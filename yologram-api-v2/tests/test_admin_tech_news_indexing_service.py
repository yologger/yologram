...
PATCH_REPO = "app.domain.search.tech.news_indexing_service.TechNewsRepository"
...
def _service(publisher: MagicMock, max_id: int | None = None):
    ...
    with patch(PATCH_REPO) as mock_repo_cls:
        ...
        return AdminTechNewsIndexingService(MagicMock(), publisher=publisher)
...
class TestIndexSingle:
    ...
    def test_from과_to를_같은_값으로_발행한다(self):
        ...
        assert publisher.publish.call_args.args[0].target == TARGET_TECH_NEWS
