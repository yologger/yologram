...
class TestTechNewsService:
    ...
    def setup_method(self):
        ...
        self.cms_client = MagicMock()
        self.mapping_repo.find_by_news_ids.return_value = []
        self.cms_client.find_category_names.return_value = {}
        ...
        self.category_patcher = patch(
            "app.domain.news.tech.service.LocalCmsApiClient", return_value=self.cms_client
        )
    ...
    def test_카테고리가_배치_조회되어_tech_category_라벨로_해석된다(self):
        ...
        self.cms_client.find_category_names.return_value = {10: "Backend", 20: "Cloud"}

        result = self.service.get_news_by_cursor(category_id=None, cursor=None, size=20)

        self.mapping_repo.find_by_news_ids.assert_called_once_with([1, 2])
        # 매핑의 category_id를 중복 없이 cms 클라이언트로 배치 해석 (N+1 회피)
        (called_ids,), _ = self.cms_client.find_category_names.call_args
    ...
    def test_삭제된_카테고리_매핑은_라벨에서_제외된다(self):
        ...
        self.cms_client.find_category_names.return_value = {10: "Backend"}
