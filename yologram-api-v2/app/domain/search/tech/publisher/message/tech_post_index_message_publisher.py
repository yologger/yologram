...
class TechPostIndexMessagePublisher(Protocol):
    """인덱싱 작업 발행 추상화 (PostViewEventPublisher와 동일한 Protocol 패턴).
    도메인 서비스는 이 인터페이스만 알고 AWS SDK(boto3)는 구현에 격리한다."""

    def publish(self, message: TechPostIndexMessage) -> None: ...


class SqsTechPostIndexMessagePublisher:
    """
    인덱싱 작업 SQS 발행 (api-v1 TechPostIndexMessagePublisher 미러).
