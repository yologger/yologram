...
class TechPostLikeService:
    """테크 게시글 좋아요 — 이력(tech_post_like)이 진실, 카운트(tech_post_like_count)는 비정규화.
    ...
    def like(self, post_id: int, uid: int) -> None:
        """좋아요 (멱등). 이력 INSERT IGNORE가 실제로 삽입한 경우에만 카운트 +1 —
        이미 누른 상태(동시 요청 uk 충돌 포함)는 0행 삽입 → 카운트 미증가로 이력과 정합 유지.
        같은 세션(get_db 요청 단위 commit)이라 이력 삽입과 카운트 증가가 원자적으로 커밋/롤백된다."""
    ...
    def unlike(self, post_id: int, uid: int) -> None:
        """좋아요 취소 (멱등). 이력 DELETE가 실제로 지운 경우에만 카운트 -1 —
