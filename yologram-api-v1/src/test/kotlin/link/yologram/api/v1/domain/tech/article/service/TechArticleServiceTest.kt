...
class TechArticleServiceTest {
    ...
    private val service = TechArticleService(techArticleRepository, mappingRepository, categoryRepository)
    ...
    fun `아티클 목록과 nextCursor를 반환한다`() {
        ...
        val result = service.getArticlesByCursor(categoryId = null, cursor = null, size = 20)
    ...
    fun `카테고리가 배치 조회되어 응답에 매핑된다`() {
        ...
        whenever(mappingRepository.findByArticleIdIn(listOf(1L, 2L))).thenReturn(
            listOf(
                TechArticleCategoryMapping(id = 1, articleId = 1L, categoryId = 2L),
                TechArticleCategoryMapping(id = 2, articleId = 1L, categoryId = 5L),
            )
        )
    ...
    fun `categoryId 필터가 리포지토리로 전달된다`() {
        ...
        service.getArticlesByCursor(categoryId = 2L, cursor = null, size = 20)

        verify(techArticleRepository).findSummarizedArticles(eq(2L), anyOrNull(), eq(20))
    }
    ...
    fun `커서가 디코딩되어 리포지토리로 전달된다`() {
        ...
        service.getArticlesByCursor(categoryId = null, cursor = cursor, size = 20)
    ...
    fun `잘못된 커서면 INVALID_CURSOR 예외가 발생한다`() {
        assertThrows<InvalidTechArticleCursorException> {
            service.getArticlesByCursor(categoryId = null, cursor = "@@@", size = 20)
        }
    ...
    fun `size는 1~50으로 보정된다`() {
        ...
        service.getArticlesByCursor(categoryId = null, cursor = null, size = 999)
        verify(techArticleRepository).findSummarizedArticles(anyOrNull(), anyOrNull(), eq(TechArticleService.MAX_PAGE_SIZE))

        service.getArticlesByCursor(categoryId = null, cursor = null, size = -1)
