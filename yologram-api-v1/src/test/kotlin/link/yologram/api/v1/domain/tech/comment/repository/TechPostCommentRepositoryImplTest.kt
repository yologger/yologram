...
@ActiveProfiles("test")
@Transactional
class TechPostCommentRepositoryImplTest {
    ...
    private fun saveComment(postId: Long, userId: Long = 1L): TechPostComment =
        commentRepository.save(TechPostComment(postId = postId, userId = userId, content = "내용"))
    ...
    inner class 글_필터 {
        ...
        fun `요청한 postId의 댓글만 반환한다`() {
            ...
            val result = commentRepository.findByPost(100L, TechPostCommentSort.LATEST, null, 10)
    ...
    inner class 정렬 {
        ...
        fun `오래된순은 id 오름차순으로 정렬한다`() {
            ...
            val result = commentRepository.findByPost(100L, TechPostCommentSort.OLDEST, null, 10)
    ...
    inner class 커서 {
        ...
        fun `최신순 커서는 cursorId보다 작은(과거) 댓글만 반환한다`() {
            ...
            val result = commentRepository.findByPost(100L, TechPostCommentSort.LATEST, cursorId, 10)
        ...
        fun `오래된순 커서는 cursorId보다 큰(이후) 댓글만 반환한다`() {
            ...
            // cursorId를 Long?로 명시 — 그러지 않으면 Kotlin이 더 구체적인 offset(Long) 오버로드를 택한다
            val cursorId: Long? = b.id
            val result = commentRepository.findByPost(100L, TechPostCommentSort.OLDEST, cursorId, 10)
    ...
    inner class 개수_제한 {
        ...
        fun `limit 개수만큼만 반환한다`() {
            ...
            val result = commentRepository.findByPost(100L, TechPostCommentSort.LATEST, null, 2)
    ...
    inner class 글_단위_삭제 {
        ...
        fun `해당 글의 댓글만 전부 삭제하고 다른 글 댓글은 보존한다`() {
            ...
            assertEquals(0, commentRepository.findByPost(100L, TechPostCommentSort.LATEST, null, 10).size)
            assertEquals(listOf(other.id), commentRepository.findByPost(200L, TechPostCommentSort.LATEST, null, 10).map { it.id })
        }
    }
}
