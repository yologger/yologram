...
@WebMvcTest(AdminTechPostIndexingResource::class)
@Import(
...
class AdminTechPostIndexingResourceTest {

    @Autowired
    lateinit var mockMvc: MockMvc

    @MockitoBean
    lateinit var adminTechPostIndexingService: AdminTechPostIndexingService
    ...
    private val baseUrl = "/api/v1/search/admin/tech/posts/indexing"
    ...
    inner class 전체_인덱싱 {
        ...
        fun `202를 반환하고 작업을 발행한다`() {
            ...
            verify(adminTechPostIndexingService).fullIndexAsync()
        }
        ...
        fun `토큰이 없으면 401이고 발행하지 않는다`() {
            ...
            verify(adminTechPostIndexingService, never()).fullIndexAsync()
        }
    ...
    inner class 단건_인덱싱 {
        ...
        fun `202를 반환하고 해당 id로 발행한다`() {
            ...
            verify(adminTechPostIndexingService).index(eq(42L))
        }
        ...
        fun `토큰이 없으면 401`() {
            ...
            verify(adminTechPostIndexingService, never()).index(eq(42L))
        }
    ...
    inner class 범위_인덱싱 {
        ...
        fun `202를 반환하고 범위로 발행한다`() {
            ...
            verify(adminTechPostIndexingService).index(from = eq(1L), to = eq(45L))
        }

        @Test
        fun `from이 to보다 크면 400`() {
            givenValidAdminToken()
            whenever(adminTechPostIndexingService.index(from = eq(30L), to = eq(10L)))
        ...
        fun `토큰이 없으면 401`() {
            ...
            verify(adminTechPostIndexingService, never()).index(from = eq(1L), to = eq(45L))
        }
    }
}
