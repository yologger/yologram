package link.yologram.api.v1.domain.tech.news.model
...
data class TechNewsCursor(
    ...
    companion object {
        ...
        fun decode(value: String): TechNewsCursor =
            try {
                val raw = String(Base64.getUrlDecoder().decode(value))
                val (publishedAt, id) = raw.split(DELIMITER, limit = 2)
                TechNewsCursor(LocalDateTime.parse(publishedAt, FORMATTER), id.toLong())
            } catch (e: Exception) {
                throw InvalidTechNewsCursorException()
            }
    }
}
