...
class SesEmailSender(
    ...
    override fun sendPasswordResetCode(to: String, code: String) {
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 20px;">
                <tr><td align="center">
                  <table width="420" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;">
                    ...
                    <tr>
                      <td style="padding:28px 36px 0;text-align:center;">
                        <p style="margin:0;font-size:16px;color:#3f3f46;">비밀번호 재설정 코드</p>
                        <p style="margin:8px 0 0;font-size:14px;color:#71717a;">아래 인증 코드를 입력해 비밀번호를 재설정하세요.</p>
                      </td>
                    </tr>
                    <tr>
                      <td style="padding:24px 36px;">
                    ...
                    <tr>
                      <td style="padding:0 36px 32px;text-align:center;">
                        <p style="margin:0;font-size:13px;color:#a1a1aa;">이 코드는 5분간 유효합니다. 본인이 요청하지 않았다면 무시하세요.</p>
                      </td>
                    </tr>
                    <tr>
                      <td style="padding:16px 36px;border-top:1px solid #e4e4e7;text-align:center;">
        ...
        val request = SendEmailRequest.builder()
            ...
            .message(
                Message.builder()
                    .subject(Content.builder().data("[yologram] 비밀번호 재설정 코드").charset("UTF-8").build())
                    .body(
                        Body.builder()
                            .html(Content.builder().data(html).charset("UTF-8").build())
                            .build()
        ...
        try {
            sesClient.sendEmail(request)
            logger.info { "[SesEmailSender] 비밀번호 재설정 to=$to" }
        } catch (e: Exception) {
            logger.error(e) { "[SesEmailSender] 비밀번호 재설정 발송 실패: to=$to" }
            throw e
        }
    }
}
