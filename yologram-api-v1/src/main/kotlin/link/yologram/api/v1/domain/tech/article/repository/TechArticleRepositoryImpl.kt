...
class TechArticleRepositoryImpl(
    ...
    override fun findSummarizedArticles(categoryId: Long?, cursor: TechArticleCursor?, limit: Int): List<TechArticle> {
        ...
        if (categoryId != null) {
            builder.and(
                JPAExpressions
                    .selectOne()
                    .from(mapping)
                    .where(mapping.articleId.eq(article.id), mapping.categoryId.eq(categoryId))
                    .exists()
            )
        }
