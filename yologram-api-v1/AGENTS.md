...
- src/main/kotlin/.../domain/search/tech/: 검색 인덱싱 요청 — publisher/message(TechIndexingMessage 계약, worker와 문자열 미러 + Publisher — SQS SendMessage, 큐 URL 캐시. 조회 이벤트와 달리 실패를 전파한다: 어드민이 명시 요청한 작업), service/AdminTechPostIndexingService(범위를 20건 청크로 쪼개 발행 — 가시성 타임아웃 초과·재시도 단위·병렬성 때문), resource(PUT /search/admin/tech/posts/indexing{,/{id},/{from}/{to}} — 전부 202 + 어드민 토큰. indexing은 조작 세그먼트 — /posts에 PUT을 걸면 게시글 수정으로 읽힌다). 스위치는 yologram.messages.publish.post-index.{enabled,queue}(MessagePublishProperties), SDK 빈은 config/sqs/SqsConfig. 전체 인덱싱만 @Async("sqsTaskExecutor")로 돌린다 — 발행 루프를 요청 스레드에서 돌리면 게시글이 늘수록 게이트웨이 타임아웃(30초)에 걸린다(로컬 실측 202/7.8ms). 실제 색인은 하지 않는다(OpenSearch 클라이언트 없음 — worker 담당)
- src/main/kotlin/.../domain/search/tech/{document,repository,service,resource}/: 게시글 검색 — GET /search/tech/posts?q&page&size&sort. multi_match(title^2·content, nori 형태소), offset 페이징(page→from), 정렬 RELEVANCE(_score→createdAt)/LATEST(createdAt→_score). alias(tech-post-index)만 참조하고 실제 인덱스명은 쓰지 않는다(무중단 재색인 전략). trackTotalHits(true) — 기본값이면 10000건에서 카운트가 고정돼 마지막 페이지가 틀어진다. 색인에 없는 닉네임·likedByMe는 응답 조립 시 배치 조회로 채운다(개인화·변경 빈도 때문에 색인 대상이 아니다 — 번장 pipeline/resultenhancer와 같은 결). 검색 비활성(opensearch.main.enabled=false)이면 503 SEARCH_UNAVAILABLE — 조건부 빈으로 막으면 404가 되어 "없는 경로"로 오해된다(api-v2와 같은 계약)
- src/main/kotlin/.../config/opensearch/: OpenSearch 클라이언트 (worker 미러) — transport에 Boot ObjectMapper 주입(기본 mapper는 Java 8 날짜를 모른다), destroyMethod는 transport에 건다(클라이언트엔 close()가 없다). 빈은 @Lazy이고 리포지토리가 ObjectProvider로 늦게 꺼낸다 — 설정이 비면 URI 파싱에서 실패하므로 기동 시점 생성을 피한다
