...
어드민 전용 JWT 로그인·인증 가드가 적용되어 있으며, 현재 어드민 관리·뉴스 소스 관리가 실화면으로 동작한다 (나머지 메뉴는 준비 중).
...
- 사이드바(하위): 현재 최상위 메뉴의 하위 항목 표시 — 예: 유저 관리 → [유저 관리, 어드민 관리], 뉴스 관리 → [기술 뉴스(뉴스 관리·소스 관리), 투자 뉴스, 정치 뉴스]
