...
describe('getArticles', () => {
  it('첫 페이지 조회 시 아티클 목록과 nextCursor를 반환한다', async () => {
    ...
    expect(page.nextCursor).toBe('article-next-cursor')
  })

  it('마지막 페이지는 nextCursor 없이 반환된다', async () => {
    const page = await getArticles({ cursor: 'article-next-cursor' })
  ...
  it('categoryId로 필터하면 해당 카테고리 아티클만 반환한다', async () => {
    const page = await getArticles({ categoryId: 3 })
  ...
  it('categoryId가 쿼리스트링으로 전달된다', async () => {
    ...
    await getArticles({ categoryId: 3 })
  ...
  it('categoryId가 null이면 categoryId 파라미터를 보내지 않는다', async () => {
    ...
    await getArticles({ categoryId: null })
  ...
  it('size 파라미터가 쿼리스트링으로 전달된다', async () => {
    ...
    await getArticles({ size: 20 })

    expect(capturedUrl).toContain('size=20')
  })

  it('잘못된 커서면 에러를 던진다 (400 INVALID_CURSOR)', async () => {
    await expect(getArticles({ cursor: 'broken-cursor' })).rejects.toThrow()
  })
})
