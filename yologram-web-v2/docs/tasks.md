...
- [x] 최상위 탭 순서 변경(기술 우선) + 기본 진입 /tech
- [x] antd App 래퍼 적용(모달/메시지 테마 일관), 링크/활성 칩 분홍 테마 통일, 필터 칩 가로 스크롤

## 기술 커뮤니티 (UI, 더미데이터)

- [x] 기술 서브탭 커뮤니티·채용 추가
- [x] 게시글/댓글 타입 + Jotai atom (더미 시드)
- [x] 피드 (/tech/community): PostCard 목록 + 무한 스크롤 + 하단 작성바 + 맨 위로 FAB
- [x] 글 작성 (/tech/community/write): 제목(optional)+내용+카테고리(최대3) + 풀스크린 오버레이
- [x] 글 상세 (/tech/community/[postId]): 본문 + 액션행 + 댓글 목록/입력 + 풀스크린 오버레이
