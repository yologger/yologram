...
- 기술 서브탭 커뮤니티·채용 추가, 최상위 탭 순서·기본 진입(/tech)
- 피드(무한 스크롤)+작성바+맨 위로 FAB, 글 작성/상세(풀스크린 오버레이)+댓글
- 카테고리: 필터(전체+7) 단일선택 / 작성 다중 태깅(최대3) / 배지
- 헤더 스크롤 collapse(SubTabLayout collapseOnScroll)
- 더미 atom(Jotai), 분홍 테마(#f2a0b5) 일관, antd App 래퍼로 모달/메시지 테마 적용
