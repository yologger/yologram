...
class TechNewsCategoryParserTest {
    ...
    fun `카테고리 섹션을 분리하고 summary에서 제거한다`() {
        ...
        val parsed = TechNewsCategoryParser.parse(output, vocabulary)
    ...
    fun `슬래시가 포함된 AI ML 라벨도 파싱된다`() {
        val parsed = TechNewsCategoryParser.parse("$body\n\n**🏷️ 카테고리**\nAI/ML", vocabulary)
    ...
    fun `마커가 없으면 기타로 폴백하고 summary는 그대로다`() {
        val parsed = TechNewsCategoryParser.parse(body, vocabulary)
    ...
    fun `어휘에 기타가 없으면 폴백은 빈 목록이다`() {
        ...
        val parsed = TechNewsCategoryParser.parse(body, withoutEtc)
