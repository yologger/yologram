 ...
 * Redis 자동구성 exclude + 커스텀 프로퍼티(cache.data.redis.*) + 수동 빈. (api-v1 RedisConfig 미러)
 * 연결은 lazy — 첫 캐시 연산 시점에 커넥션을 맺으므로 Redis 미기동 환경(테스트 등)에서도 부팅에 영향 없음.
 * 캐시 무효화 발행은 부가 기능 — Redis 장애가 배치 본 작업을 막으면 안 되므로 짧은 타임아웃 + 즉시 거부.
 *
...
class RedisConfig(private val properties: CacheRedisProperties) {
    ...
    fun redisConnectionFactory(clientResources: ClientResources): LettuceConnectionFactory {
        // 캐시 무효화는 부가 기능 — Redis 장애가 배치 지연으로 전파되지 않도록 짧은 타임아웃 + 끊김 시 즉시 거부.
        // (기본값이면 command timeout 60초가 배치를 그만큼 붙잡음)
