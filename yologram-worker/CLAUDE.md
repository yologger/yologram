...
- 테이블: tech_article_source + tech_article. 전 테이블 FK 미사용(같은 도메인 포함), status(COLLECTED/SUMMARIZED/FAILED)+retry_count가 작업 큐
...
- HTTP 호출은 WebClient 통일(WebClientFactory). LLM 모델 선정은 실측 기준(/v1/models 조회, 429 quotaValue) — docs/rules.md 참조
- SQS 컨슈머는 EventHandler(canHandle/handle) 라우팅 패턴으로 (해당 기능 진행 시)

## 설정 관리

- application-local.yaml: 로컬 (포트 5003, OTLP 비활성, Parameter Store optional — DB는 worker_local SSM)
