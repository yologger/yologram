...
- domain/search/tech/: 검색 인덱싱 — subscriber/message(TechIndexingMessage 계약 + TechIndexingMessageSubscriber — SQS 수동 ack, 색인 성공 후에만 삭제. 큐는 하나이고 target으로 게시글·뉴스를 가른다), service/TechPostIndexService·TechNewsIndexService(범위 조회 → bulk 색인, 문서 id=원본 id로 멱등. @PostConstruct로 인덱스·alias upsert), document/. 인덱스는 tech-post-index-v2·tech-news-index-v1 + 각 alias(매핑 변경 시 새 버전 재색인 → _reindex → alias 원자적 이동으로 무중단)
- 뉴스 색인 경로는 둘이다: 요약 배치가 끝나면 그 배치에서 SUMMARIZED가 된 id들을 직접 색인(배치당 1회·커밋 이후·실패 삼킴 — 캐시 무효화와 같은 원칙), 어드민 요청 범위는 SQS로 받는다. 색인 대상은 SUMMARIZED만(COLLECTED는 summary가 없다). 뉴스는 worker 소유 도메인이라 리포지토리를 직접 쓴다 — 게시글이 infra/client/pms를 경유한 것은 pms가 api 소유라 경계를 넘기 때문
