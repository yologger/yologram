...
- [x] (Search) 검색 정확도 개선 — standard 서브필드 + operator AND (인덱스 v2)
  - 증상: "마이그레이션" 검색에 56건이 나오고 Kafka·Docker 등 무관한 글이 섞였다
  - 원인 ①nori 사전에 없는 외래어를 문맥마다 다르게 쪼갠다 — 같은 "마이그레이션"이 `그레|이`(단독), `마이|그레|이`(SQLAlchemy 2.0 뒤), `마|이|그레이|션`(TypeScript 5.5 뒤)로 갈렸다. 그래서 쿼리 옵션만으로는 못 잡는다(AND로 바꿔도 검색어 토큰과 문서 토큰이 달라 TypeScript 글이 빠졌다) ②multi_match 기본 operator가 OR이라 한 글자 토큰 "이" 하나에 대량 매칭
  - 해결: title·content에 standard 서브필드를 추가해 단어를 통째로도 색인하고(nori는 조사·어미, standard는 외래어 담당) 검색은 네 필드를 함께 본다. 여기에 operator AND를 더했다. 임시 인덱스로 사전 검증 후 적용 — 56건 → 2건, 조사 포함 검색("검색 기능을")은 회귀 없음
  - 번장 bun-search-indexer는 nori가 아니라 ngram 분석기를 쓴다(name·description). 사전이 필요 없어 외래어 문제가 없지만 상품명처럼 짧은 텍스트 전제라, 본문이 긴 게시글에 적용하면 인덱스가 커진다 — 최소 변경으로 standard 서브필드만 택했다
  - 매핑 변경이라 인덱스 v2로 올렸고 전환은 무중단으로 됐다: _reindex API로 v1 → v2를 앱 없이 복사(86건 132ms)한 뒤 alias만 원자적으로 이동(remove v1 + add v2 한 요청). "색인이 alias로 나가니 무중단 불가"라던 처음 판단은 앱 재색인만 고려한 것이었다 — 엔진이 직접 복사하면 앱을 거치지 않는다
