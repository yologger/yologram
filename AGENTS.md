...
- 에이전트 지침의 단일 소스는 AGENTS.md다(루트·각 프로젝트 모두). CLAUDE.md는 AGENTS.md를 가리키는 포인터일 뿐이라 내용을 두지 않는다 — 지침 추가·수정은 AGENTS.md만 고친다(두 파일 동일 유지 규칙은 중복 관리 비용 때문에 폐기)
- 코드 개발/수정 시 관련 문서(README.md, AGENTS.md, 루트 docs/)를 함께 최신화할 것
...
- 코드 구현 시 신규라이브러리나 기술이 추가되면, 해당 프로젝트의 README.md에도 반영한다.
