...
describe('TechArticlesPage', () => {
  it('아티클 목록(출처·카테고리 태그·제목·요약)이 렌더링된다', async () => {
    renderWithProviders(<TechArticlesPage />)

    expect(await screen.findByText('첫 번째 아티클')).toBeInTheDocument()
    expect(screen.getByText('두 번째 아티클')).toBeInTheDocument()
  ...
  it('제목은 원문 링크로 새 탭에서 열린다 (noopener noreferrer)', async () => {
    ...
    const link = await screen.findByRole('link', { name: '첫 번째 아티클' })
  ...
  it('카테고리 API에서 받은 칩이 표시되고 선택 시 해당 categoryId로 재조회한다', async () => {
    ...
    await screen.findByText('첫 번째 아티클')
    ...
    // 'Frontend'(id=1) 칩 클릭 → Frontend 아티클만 남음
    await user.click(screen.getAllByText('Frontend')[0])

    expect(await screen.findByText('두 번째 아티클')).toBeInTheDocument()
    expect(screen.queryByText('첫 번째 아티클')).not.toBeInTheDocument()
  })
  ...
  it('슬래시 포함 라벨(AI/ML) 칩도 categoryId로 정상 필터링한다', async () => {
    ...
    expect(await screen.findByText('AI 아티클')).toBeInTheDocument()
    expect(screen.queryByText('첫 번째 아티클')).not.toBeInTheDocument()
  })
  ...
  it('센티널 교차 시 다음 페이지를 이어 붙이고 마지막 페이지면 스크롤을 멈춘다', async () => {
    ...
    expect(await screen.findByText('다음 페이지 아티클')).toBeInTheDocument()
    // 기존 페이지 유지
    expect(screen.getByText('첫 번째 아티클')).toBeInTheDocument()
    ...
    expect(screen.getAllByText('다음 페이지 아티클').length).toBe(1)
  })

  it('아티클이 없으면 빈 상태 문구를 표시한다', async () => {
    server.use(
      http.get('http://localhost:5001/api/v1/articles/tech', () =>
  ...
  it('조회 실패 시 에러 문구와 다시 시도 버튼을 표시하고, 다시 시도로 재조회한다', async () => {
    ...
    expect(await screen.findByText(/아티클을 불러오지 못했어요/)).toBeInTheDocument()
